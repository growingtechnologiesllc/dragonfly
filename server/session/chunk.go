...
func (s *Session) subChunkEntry(offset protocol.SubChunkOffset, ind int16, col *world.Column, transaction map[uint64]struct{}) protocol.SubChunkEntry {
	...
	subMapType := byte(protocol.HeightMapDataHasData)
	var subMap protocol.HeightMap
	...
	hasSubMap := true
	if higher {
		subMapType, hasSubMap = protocol.HeightMapDataTooHigh, false
	} else if lower {
		subMapType, hasSubMap = protocol.HeightMapDataTooLow, false
	}
